...
namespace get_link_manga
{
    public partial class MainWindow : Window
    {
        ...
        private static readonly SiteDownloadProfile[] _downloadProfiles = new[]
        {
            ...
            {
                Id = "damconuong",
                HostAliases = new[] { "damconuong.shop", "damconuong.store" },
                BrowserSessionPreferred = false,
                ChromeFallbackPreferred = false,
                DefaultConcurrencyCap = 4,
                InterRequestDelayMs = 120,
            ...
            {
                Id = "nettruyen",
                HostAliases = new[] { "nettruyen", "nettruyenviet" },
                BrowserSessionPreferred = false,
                ChromeFallbackPreferred = false,
                DefaultConcurrencyCap = 6,
                InterRequestDelayMs = 100,
        ...
        private void CacheBrowserSessionForHost(string urlOrHost, BrowserSessionSnapshot snapshot)
        {
            if (snapshot == null)
            {
                return;
            ...
            _browserSessionSnapshots[host] = snapshot;
        }

        private void RememberBrowserSession(string originalUrl, Uri resolvedUri, string userAgent, CookieContainer cookies, BrowserSessionEngine engine)
        {
            ...
            BrowserSessionSnapshot snapshot = BuildBrowserSessionSnapshot(resolvedUri, userAgent, cookies, engine);
            CacheBrowserSession(snapshot);
            CacheBrowserSessionForHost(originalUrl, snapshot);
        ...
        private void ApplyBrowserSessionSnapshot(BrowserSessionSnapshot snapshot)
        {
            ...
            {
                RememberScopedUserAgent(snapshot.SourceHost, snapshot.UserAgent);
                RememberScopedUserAgent(snapshot.ResolvedUrl, snapshot.UserAgent);
            }
            ...
            MergeBrowserSessionCookiesIntoScopedContainer(snapshot.SourceHost, resolvedUri, snapshot.Cookies);
            if (resolvedUri != null && !string.Equals(snapshot.SourceHost, resolvedUri.Host, StringComparison.OrdinalIgnoreCase))
            {
                MergeBrowserSessionCookiesIntoScopedContainer(resolvedUri.Host, resolvedUri, snapshot.Cookies);
            }
        }

        private void MergeBrowserSessionCookiesIntoScopedContainer(string urlOrHost, Uri fallbackUri, IEnumerable<BrowserSessionCookie> cookies)
        {
            if (cookies == null)
            {
                return;
            ...
            foreach (var cookie in cookies)
            {
                ...
                string domain = string.IsNullOrWhiteSpace(cookie.Domain) ? fallbackUri?.Host : cookie.Domain;
                var netCookie = new System.Net.Cookie(cookie.Name, cookie.Value ?? string.Empty, string.IsNullOrWhiteSpace(cookie.Path) ? "/" : cookie.Path, domain ?? string.Empty)
                {
            ...
            MergeCookiesIntoScopedContainer(urlOrHost, fallbackUri, netCookies);
        }
        ...
        private async Task<BrowserSessionSnapshot> AcquireBrowserSessionAsync(string url, SiteDownloadProfile profile, CancellationToken token)
        {
            ...
            {
                Log($"Chrome fallback bat dau cho {ExtractHostOrRaw(url)} sau khi WebView2 khong du session.", LogSeverity.Warning, "Browser Session");
        ...
        private async Task<BrowserSessionSnapshot> AcquireChromeFallbackSessionAsync(string url, CancellationToken token)
        {
            return await Task.Run(() =>
            {
                try
                {
                    Log($"Khoi dong Selenium Chrome fallback cho {ExtractHostOrRaw(url)}.", LogSeverity.Warning, "Browser Session");
                    ...
                    using (var driver = new ChromeDriver(service, options, TimeSpan.FromMinutes(2)))
                    {
                        ...
                        Log($"Chrome fallback lay session thanh cong cho {resolvedUri.Host}.", LogSeverity.Info, "Browser Session");
                        return snapshot;
                    }
                }
                catch (Exception ex)
                {
                    Log($"Chrome fallback that bai: {ex.Message}", LogSeverity.Error, "Browser Session");
                    return null;
                }
            }, token);
        }
        ...
        private async Task<string> GetHtmlWithSiteSessionAsync(string url, SiteDownloadProfile profile, CancellationToken token)
        {
            ...
            using (var retryHttpClient = CreateScopedHttpClient(url))
            using (var retryResponse = await retryHttpClient.SendAsync(retryRequest, HttpCompletionOption.ResponseHeadersRead, token))
            {
