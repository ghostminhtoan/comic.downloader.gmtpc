    ...
    public partial class MainWindow : Window
    {
        ...
        private async Task ScrapeTruyenggvnAsync(bool clearExisting)
        {
            ...
            try
            {
                ...
                for (int page = pageFrom; page <= pageTo; page++)
                {
                    ...
                    UpdateResultsCrawlProgress(processed, totalPages, GuessImportDisplayName(normalizedUrl));
        ...
        private async Task DownloadTruyenggvnGalleryAsync(GalleryItem item, string rootFolder, CancellationToken token, GalleryItem queueItem = null, ChapterFilter chapterFilter = null)
        {
            ...
            if (chapterFilter != null)
            {
                ...
                chapterLinks = FilterPendingChapterLinksFromProcess(rootFolder, TruyenggvnSiteFolder, item, filtered);
                if (chapterLinks.Count == 0)
                {
                    TruyenggvnLog($"Không có chapter nào cần tải (hoặc đã hoàn tất theo process) trong tổng {totalFound} chapter của '{item.Name}'.");
                    if (queueItem != null)
                    {
                        Dispatcher.BeginInvoke((Action)(() =>
                        {
        ...
        private double ParseTruyenggvnChapterNumber(string url)
        {
            ...
            TruyenggvnChapterRouteInfo routeInfo = ParseTruyenggvnChapterRouteInfo(url);
            if (routeInfo != null &&
                !string.IsNullOrWhiteSpace(routeInfo.ChapterToken) &&
                TryParseChapterNumberFromChapterToken(routeInfo.ChapterToken, out double strictValue))
            {
                return strictValue;
            }
