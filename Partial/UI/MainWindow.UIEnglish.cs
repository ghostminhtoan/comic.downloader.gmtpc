...
namespace get_link_manga
{
    public partial class MainWindow : Window
    {
        private static readonly Dictionary<string, string> UiTranslations = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["Convert"] = "Chuyển đổi",
            ["Copy file gif"] = "Copy file gif",
            ["Rename"] = "Đổi tên",
            ["Convert Tools"] = "Công cụ Convert",
            ["Copy GIF Files"] = "Copy file GIF",
            ["XnConvert Tool"] = "Công cụ XnConvert",
            ["Root folder path:"] = "Đường dẫn folder tổng:",
            ["Converted folder path:"] = "Đường dẫn folder đã convert:",
            ["SCAN GIF"] = "QUÉT GIF",
            ["COPY PATH"] = "COPY PATH",
            ["CONVERT XNCONVERT"] = "convert selected checkboxes with xnconvert",
            ...
            ["Exclude these keywords (separated by semicolon)"] = "Loại trừ các truyện chứa các từ này (ngăn cách bằng dấu chấm phẩy)",
            ["STATUS"] = "Trạng thái",
            ["PROCESS"] = "Tiến trình",
            ["VIEW LINK"] = "Mở nhanh",
            ["VIEW"] = "Xem",
            ...
            ["COPY NAME + LINK"] = "Sao chép tên + link",
            ["COPY BOOK LINKS"] = "Sao chép link truyện",
            ["COPY BOOKS NAME + LINK"] = "Sao chép tên + link truyện",
            ["SAVE LIST"] = "Lưu danh sách",
            ["LOAD LIST"] = "Tải danh sách",
            ["CLEAR"] = "Xóa",
            ["CHECK ALL"] = "Tích tất cả",
            ["UNCHECK ALL"] = "Bỏ tích tất cả",
            ["REFRESH STATUS"] = "LÀM MỚI TRẠNG THÁI",
            ["RESCAN MISSING CHAPTER"] = "QUÉT LẠI CHAP THIẾU",
            ...
            ["MERGE MD BOOK"] = "Gộp MD",
            ["COMIC BOOKS"] = "Truyện tranh",
            ["BROWSE"] = "Duyệt",
            ["AUTO SPLIT CHAPTER"] = "TỰ ĐỘNG TÁCH CHƯƠNG",
            ["APPLY"] = "ÁP DỤNG",
            ["AUTO MERGE CHAPTER"] = "GỘP TRUYỆN SPLIT",
            ["Split Chapters to Parallel Tasks"] = "Tách chương thành tác vụ song song",
            ...
            ["Clear Cache JSON"] = "Xóa cache JSON (Clear Cache JSON)",
            ...
            ["Copy missing chapter"] = "Sao chép chap thiếu",
            ["Copy all book's missing chapter"] = "Sao chép chap thiếu của mọi truyện",
            ["Google all book's missing chapter"] = "Google chap thiếu của mọi truyện",
            ...
            ["MERGE SINGLE COMIC FOLDERS"] = "Gộp thư mục truyện đơn",
            ["MERGE ROOT"] = "Thư mục gốc gộp",
            ["MERGE FOLDER"] = "Gộp thư mục",
            ["DUPLICATE DETAILS"] = "Chi tiết trùng",
            ["Ready to analyze."] = "Sẵn sàng phân tích.",
            ["SHUTDOWN AFTER DONE"] = "TẮT MÁY SAU KHI XONG",
            ["POWER"] = "TẮT MÁY",
            ["DOWNLOAD FOLDER TYPE"] = "Kiểu thư mục tải",
            ["DOWNLOAD PATH"] = "Đường dẫn tải",
            ["CHECK ERROR"] = "Tích lỗi",
            ["CLEAR COOKIE"] = "Xóa cookie",
            ["RESET COOKIE"] = "Đặt lại cookie",
            ["SHOW FLOAT BUTTON"] = "Hiện nút nổi",
            ["Hide left panel"] = "Ẩn panel trái",
            ["Pin window"] = "Ghim cửa sổ",
            ["Move window"] = "Di chuyển cửa sổ",
            ["HISTORY"] = "Lịch sử",
            ["HOME"] = "Trang chủ",
            ["DAY"] = "NGÀY",
            ["HOUR"] = "GIỜ",
        ...
        private static readonly Dictionary<string, string> LegacyVietnameseCleanup = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["Quét lại chương thiếu (Rescan Missing Chapter)"] = "Quét lại chương thiếu",
            ["Tạm dừng quét chương thiếu (Pause Scan)"] = "Tạm dừng quét chương thiếu",
            ["Mở cache JSON (Open Cache JSON)"] = "Mở cache JSON",
            ["Xóa cache JSON (Clear Cache JSON)"] = "Xóa cache JSON",
        ...
        {
            PlaySoundResource("Startup.wav");
            InitializePostDownloadUI();
        ...
        {
            if (e.Source != sender) return;
            if (ReferenceEquals(sender, tabLightNovel) ||
                (ReferenceEquals(sender, tabDownloadRoot) && IsNovelDownloadTabSelected()))
            {
        ...
        {
            Brush activeBrush = TryFindResource("CyberpunkCyanBrush") as Brush ?? Brushes.Cyan;
            Brush inactiveBrush = TryFindResource("CyberpunkMutedTextBrush") as Brush ?? Brushes.Gray;

            if (tglLanguage != null && tglLanguage.IsChecked != _isVietnameseUi)
            {
                tglLanguage.IsChecked = _isVietnameseUi;
            }

            if (txtLanguageLabel != null)
            {
                txtLanguageLabel.Text = "ENG";
                txtLanguageLabel.Foreground = _isVietnameseUi ? inactiveBrush : activeBrush;
            }

            if (txtLanguageTarget != null)
            {
                txtLanguageTarget.Text = "VI";
                txtLanguageTarget.Foreground = _isVietnameseUi ? activeBrush : inactiveBrush;
            }
        }
        ...
        private void ApplyEnglishUiText()
        {
            ...
            if (btnShutdownMenu != null) btnShutdownMenu.ToolTip = "Finish options";
            if (txtShutdownPopupHeader != null) txtShutdownPopupHeader.Text = "FINISH OPTIONS";
            if (chkShutdownAfterCompleted != null) chkShutdownAfterCompleted.Content = "shutdown after completed";
            if (lblPostDownloadCommand != null) lblPostDownloadCommand.Text = "RUN COMMAND (Win+R style):";
            if (lblPostDownloadShortcut != null) lblPostDownloadShortcut.Text = "RUN SHORTCUT:";
            if (btnApplyShortcut != null) btnApplyShortcut.Content = "APPLY";
            if (btnClearShortcut != null) btnClearShortcut.Content = "CLEAR";
            ...
            if (tglAutoScrollDownloading != null) tglAutoScrollDownloading.ToolTip = "AUTO SCROLL TO DOWNLOADING";
            if (lblAutoScrollDownloading != null) lblAutoScrollDownloading.ToolTip = "AUTO SCROLL TO DOWNLOADING";
