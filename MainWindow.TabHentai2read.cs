...
namespace get_link_manga
{
    public partial class MainWindow : Window
    {
        ...
        private IEnumerable<GalleryItem> ParseHentai2readGalleryItems(string html)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                if (slug.Equals("hentai-list", StringComparison.OrdinalIgnoreCase) ||
                    ...
                    slug.Equals("login", StringComparison.OrdinalIgnoreCase) ||
                    slug.Equals("register", StringComparison.OrdinalIgnoreCase) ||
                    slug.Equals("ranking", StringComparison.OrdinalIgnoreCase) ||
                    slug.Equals("faqs", StringComparison.OrdinalIgnoreCase) ||
                    slug.Equals("hentai-search", StringComparison.OrdinalIgnoreCase) ||
                    slug.StartsWith("tag-", StringComparison.OrdinalIgnoreCase) ||
                    slug.StartsWith("tags", StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }
        ...
        private async Task ScrapeHentai2readAsync(bool clearExisting)
        {
            ...
            try
            {
                ...
                for (int page = pageFrom; page <= pageTo; page++)
                {
                    ...
                    UpdateResultsCrawlProgress(page - pageFrom + 1, totalPages, GuessImportDisplayName(route.NormalizedListingUrl));
        ...
        private async Task DownloadHentai2readBookAsync(GalleryItem item, string rootFolder, CancellationToken token, GalleryItem queueItem, ChapterFilter chapterFilter)
        {
            ...
            if (chapterFilter != null)
            {
                var filtered = chapterLinks
                    ...
                    .ToList();
                chapterLinks = FilterPendingChapterLinksFromProcess(rootFolder, Hentai2readSiteFolder, item, filtered);
            }
            else
            {
        ...
        private void BtnHentai2readShowLog_Click(object sender, RoutedEventArgs e)
        {
            ShowProjectWindowForSection(AppSection.TraceLog);
        }
    }
}
