    ...
    public partial class MainWindow : Window
    {
        ...
        internal async Task<bool> SolveTruyenqqCaptchaIfNeededAsync(string testUrl)
        {
            ...
            if (cachedSession != null)
            {
                ...
                if (string.IsNullOrWhiteSpace(_truyenqqPreferredBaseUrl) &&
                    Uri.TryCreate(cachedSession.ResolvedUrl, UriKind.Absolute, out Uri cachedResolvedUri))
                {
                    _truyenqqPreferredBaseUrl = $"{cachedResolvedUri.Scheme}://{cachedResolvedUri.Host}";
                    testUrl = ResolveTruyenqqRequestUrl(testUrl);
                }
            }
                ...
                for (int page = pageFrom; page <= pageTo; page++)
                {
                    ...
                    foreach (Match match in viewMatches)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                string previewUrl = ExtractTruyenqqPreviewUrlFromHtml(rawContent, fullLink);
            ...
            try
            {
                for (int i = 0; i < total; i++)
                {
                    ...
                    try
                    {
                        ...
                        string previewUrl = ExtractTruyenqqPreviewUrlFromHtml(html, link);
        ...
        private string ExtractTruyenqqPreviewUrlFromHtml(string html, string pageUrl)
        {
            return GetTruyenqqPreviewUrlCandidatesFromHtml(html, pageUrl).FirstOrDefault() ?? string.Empty;
        }

        private List<string> GetTruyenqqPreviewUrlCandidatesFromHtml(string html, string pageUrl)
        {
            ...
            Match avatarBlockMatch = Regex.Match(
                html,
                @"<div[^>]*class=[""'][^""']*\bbook_avatar\b[^""']*[""'][^>]*>(?<content>[\s\S]*?)</div>",
                RegexOptions.IgnoreCase);
            if (avatarBlockMatch.Success)
            {
                avatarHtml = avatarBlockMatch.Groups["content"].Value;
            }

            CollectTruyenqqPreviewUrls(avatarHtml, pageUrl, urls);
            if (urls.Count == 0)
            {
                CollectTruyenqqPreviewUrls(html, pageUrl, urls);
            }

            return urls;
        }

        private static void CollectTruyenqqPreviewUrls(string htmlFragment, string pageUrl, List<string> urls)
        {
            ...
            foreach (Match match in Regex.Matches(
                htmlFragment,
                @"(?:src|data-ni)=[""'](?<url>[^""']+?\.(?:jpe?g|png|gif|webp|bmp)(?:\?[^""']*)?)[""']",
                RegexOptions.IgnoreCase))
            {
                string normalizedUrl = NormalizeTruyenqqPreviewUrl(match.Groups["url"].Value, pageUrl);
        ...
        private static string NormalizeTruyenqqPreviewUrl(string imageUrl, string pageUrl)
        {
        ...
        private async Task DownloadTruyenqqGalleryAsync(GalleryItem item, string rootFolder, CancellationToken token, GalleryItem queueItem = null, ChapterFilter chapterFilter = null)
        {
            ...
            if (segments.Length >= 2 && segments[0].Equals("truyen-tranh", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (TryGetCachedDownloadChapterLinks(item, out List<string> cachedChapterLinks))
                {
                    ...
                    TruyenqqLog($"Dùng {cachedChapterLinks.Count} chapter từ cache check thiếu chap cho '{item.Name}'.");
                    List<string> effectiveChapterLinks = chapterFilter != null
                        ? FilterPendingChapterLinksFromProcess(rootFolder, "truyenqq", item, cachedChapterLinks.Where(link => chapterFilter.IsMatch(ParseChapterNumber(link))).ToList())
                        : FilterPendingChapterLinksFromProcess(rootFolder, "truyenqq", item, cachedChapterLinks);
                    ...
                    await DownloadTruyenqqPendingChaptersAsync(item, rootFolder, token, queueItem, effectiveChapterLinks);
                    Dispatcher.Invoke(() => item.LinkCount = cachedChapterLinks.Count.ToString());
                    return;
                }
                ...
                {
                    Log($"[truyenqq] Lỗi: Không tìm thấy chương nào trong '{item.Name}'. Hủy tải xuống.");
                    if (queueItem != null)
                    {
                ...
                if (chapterFilter != null)
                {
                    ...
                    chapterLinks = FilterPendingChapterLinksFromProcess(rootFolder, "truyenqq", item, filtered);
                    if (chapterLinks.Count == 0)
                    {
                        Log($"[truyenqq] Không có chương nào cần tải (hoặc đã hoàn tất theo process) trong tổng số {totalFoundChapters} chương của '{item.Name}'.");
                        if (queueItem != null)
                        {
                            Dispatcher.Invoke(() => {
                                queueItem.Status = "Completed";
        ...
        private double ParseChapterNumber(string url)
        {
            ...
            return TryParseDownloadChapterNumberFromLink(url, out double number)
                ? number
                : 0d;
        }
