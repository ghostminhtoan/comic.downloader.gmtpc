        ...
        private async Task DownloadViHentaiGalleryAsync(GalleryItem item, string rootFolder, CancellationToken token, GalleryItem queueItem = null, ChapterFilter chapterFilter = null)
        {
            ...
            if (segments.Length == 2)
            {
                ...
                if (chapterFilter != null)
                {
                    ...
                    chapterLinks = FilterPendingChapterLinksFromProcess(rootFolder, "vi-hentai.pro", item, filtered);
                    if (chapterLinks.Count == 0)
                    {
                        Log($"[vi-hentai.pro] Không có chương nào cần tải (hoặc đã hoàn tất theo process) trong tổng số {totalFoundChapters} chương của '{item.Name}'.");
                        if (queueItem != null)
                        {
                            Dispatcher.Invoke(() => {
                                queueItem.Status = "Completed";
