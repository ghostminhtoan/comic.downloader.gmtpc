...
namespace get_link_manga
{
    public partial class MainWindow : Window
    {
        ...
        private sealed class GalleryItemState
        {
            ...
            [DataMember(Order = 23)]
            public string MissingChapterStatusText { get; set; }

            [DataMember(Order = 24)]
            public string MissingChapterSortText { get; set; }

            [DataMember(Order = 25)]
            public bool HasMissingChapterIssue { get; set; }

            [DataMember(Order = 26)]
            public string MissingChapterChapterLabel { get; set; }

            [DataMember(Order = 27)]
            public string MissingChapterLabel { get; set; }

            [DataMember(Order = 28)]
            public string MissingChapterDecimalLabel { get; set; }

            [DataMember(Order = 29)]
            public bool MissingChapterIsChecked { get; set; } = true;

            [DataMember(Order = 30)]
            public string ChapterSelectionText { get; set; }

            [DataMember(Order = 31)]
            public bool IsParallelSplitTask { get; set; }

            [DataMember(Order = 32)]
            public string MissingChapterLatestChapterText { get; set; }

            [DataMember(Order = 33)]
            public bool IsParallelSplitParent { get; set; }

            [DataMember(Order = 34)]
            public bool IsParallelSplitCollapsed { get; set; } = true;
        ...
        private sealed class GalleryMarkdownSettingsState
        {
            ...
            [DataMember(Order = 6)]
            public int AutoSplitChapters { get; set; }
        ...
        [DataContract]
        private sealed class DownloadMissingChapterCachePayload
        {
            [DataMember(Order = 1)]
            public List<DownloadMissingChapterCacheBookState> Books { get; set; } = new List<DownloadMissingChapterCacheBookState>();
        }

        [DataContract]
        private sealed class DownloadMissingChapterCacheBookState
        {
            [DataMember(Order = 1)]
            ...
            [DataMember(Order = 8)]
            public List<DownloadMissingChapterCacheChapterState> Chapters { get; set; } = new List<DownloadMissingChapterCacheChapterState>();
        }

        [DataContract]
        private sealed class DownloadMissingChapterCacheChapterState
        {
            [DataMember(Order = 1)]
            public string Name { get; set; }
        ...
        private List<GalleryItem> GetItemsToExport()
        {
            ...
            var checkedItems = flattened.Where(item => item.IsChecked).ToList();
            return checkedItems.Any() ? checkedItems : flattened;
        }
        ...
        private void SaveGalleryItemsMarkdownFile(string path, IList<GalleryItem> items, string title)
        {
            ...
            var safeItems = NormalizeGalleryItemsForState(items ?? Array.Empty<GalleryItem>());
            var summaryItems = safeItems.SelectMany(ExpandGalleryItemForSummary).ToList();
            ...
            sb.AppendLine("| No. | Checked | Name | Link | Chapter Filter | Missing Chapter | Active Chapter | Page | Status | Process | Error Count |");
            sb.AppendLine("| :--- | :---: | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :---: |");
            ...
            for (int i = 0; i < summaryItems.Count; i++)
            {
                ...
                string chapterFilter = EscapeMarkdownCell(item.ChapterSelectionText);
                string missingChapter = EscapeMarkdownCell(item.MissingChapterStatusText);
                string chapter = EscapeMarkdownCell(item.DownloadingChapter);
                string page = EscapeMarkdownCell(item.DownloadingPageProgress);
                sb.AppendLine($"| {i + 1} | {checkedStr} | {safeName} | {safeLink} | {chapterFilter} | {missingChapter} | {chapter} | {page} | {status} | {process} | {item.GetUniqueErrorCount()} |");
            }
        ...
        private List<GalleryItem> LoadGalleryItemsFromMarkdown(string content)
        {
            ...
            {
                return MaterializeGalleryItemsForDisplay(stateItems
        ...
        {
            ReaderChapterIssueItem missingChapterRow = _downloadMissingChapterRows.FirstOrDefault(candidate =>
                candidate != null &&
                ((!string.IsNullOrWhiteSpace(item?.Link) &&
                  string.Equals((candidate.BookLink ?? string.Empty).Trim(), (item.Link ?? string.Empty).Trim(), StringComparison.OrdinalIgnoreCase)) ||
                 (!string.IsNullOrWhiteSpace(item?.Name) &&
                  string.Equals((candidate.BookName ?? string.Empty).Trim(), (item.Name ?? string.Empty).Trim(), StringComparison.OrdinalIgnoreCase))));
            ...
            return new GalleryItemState
            {
                ...
                MultiDownloadCount = GetComboBoxSelectedInt(cmbMultiDownload, 2),
                MissingChapterStatusText = item.MissingChapterStatusText,
                MissingChapterSortText = item.MissingChapterSortText,
                HasMissingChapterIssue = item.HasMissingChapterIssue,
                MissingChapterLatestChapterText = item.MissingChapterLatestChapterText,
                MissingChapterChapterLabel = missingChapterRow?.ChapterLabel ?? string.Empty,
                MissingChapterLabel = missingChapterRow?.MissingChapterLabel ?? string.Empty,
                MissingChapterDecimalLabel = missingChapterRow?.DecimalChapterLabel ?? string.Empty,
                MissingChapterIsChecked = missingChapterRow?.IsChecked ?? item.IsChecked,
                ChapterSelectionText = item.ChapterSelectionText,
                IsParallelSplitTask = item.IsParallelSplitTask,
                IsParallelSplitParent = item.IsParallelSplitParent,
                IsParallelSplitCollapsed = item.IsParallelSplitCollapsed,
        ...
        private GalleryItem CreateGalleryItemFromState(GalleryItemState state)
        {
            ...
            var item = new GalleryItem
            {
                ...
                ChapterSelectionText = state.ChapterSelectionText,
                IsParallelSplitTask = state.IsParallelSplitTask,
                IsParallelSplitParent = state.IsParallelSplitParent,
                MissingChapterStatusText = state.MissingChapterStatusText,
            ...
            if (state.ParallelSplitChildren != null && state.ParallelSplitChildren.Count > 0)
            {
                item.ParallelSplitChildren = state.ParallelSplitChildren
                    .OrderBy(child => child.OriginalIndex)
                    .Select(CreateGalleryItemFromState)
                    .Where(child => child != null)
                    .ToList();
                item.IsParallelSplitParent = true;
                item.IsParallelSplitCollapsed = state.IsParallelSplitCollapsed;
                foreach (var child in item.ParallelSplitChildren)
                {
                    child.IsParallelSplitTask = true;
                    child.IsParallelSplitParent = false;
                }
            }
            return item;
        }

        private List<GalleryItem> NormalizeGalleryItemsForState(IEnumerable<GalleryItem> items)
        {
            var roots = new List<GalleryItem>();
            var childSet = new HashSet<GalleryItem>();
            foreach (var item in items ?? Enumerable.Empty<GalleryItem>())
            {
                if (item?.ParallelSplitChildren == null) continue;
                foreach (var child in item.ParallelSplitChildren.Where(child => child != null))
                {
                    childSet.Add(child);
                }
            }
            ...
            foreach (var item in items ?? Enumerable.Empty<GalleryItem>())
            {
                if (item == null || childSet.Contains(item))
                {
                    continue;
                }
        ...
        private IEnumerable<GalleryItem> ExpandGalleryItemForSummary(GalleryItem item)
        {
            if (item == null)
            {
                yield break;
        ...
        private List<GalleryItem> MaterializeGalleryItemsForDisplay(IList<GalleryItem> items)
        {
            var result = new List<GalleryItem>();
            foreach (var item in NormalizeGalleryItemsForState(items))
            {
                result.Add(item);
                if (item.HasParallelSplitChildren && !item.IsParallelSplitCollapsed)
                {
                    result.AddRange(item.ParallelSplitChildren.Where(child => child != null));
                }
            }
                ...
                using (var ms = new MemoryStream(Encoding.UTF8.GetBytes(json)))
                {
                    ...
                    if (settings != null)
                    {
                        ...
                        Dispatcher.Invoke(() => {
                            ...
                            if (cmbAutoSplitChapters != null)
                            {
                                int autoSplit = settings.AutoSplitChapters;
                                string autoSplitStr = autoSplit > 0 ? autoSplit.ToString() : "OFF";
                                foreach (ComboBoxItem item in cmbAutoSplitChapters.Items)
                                {
                                    if (string.Equals(item.Content?.ToString(), autoSplitStr, StringComparison.OrdinalIgnoreCase))
                                    {
                                        cmbAutoSplitChapters.SelectedItem = item;
                                        break;
                                    }
                                }
                            }
        ...
        {
            SelectAppSection(AppSection.FinishOptions);
        }
        ...
        private void BtnScheduleShutdownTimer_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                if (_appShutdownTimer != null)
                {
                    _appShutdownTimer.Stop();
                }

                _shutdownTargetTime = DateTime.Now.AddSeconds(totalSeconds);
                _appShutdownTimer = new DispatcherTimer();
                _appShutdownTimer.Interval = TimeSpan.FromSeconds(1);
                _appShutdownTimer.Tick += (s, ev) =>
                {
                    var remaining = _shutdownTargetTime - DateTime.Now;
                    if (remaining.TotalSeconds <= 0)
                    {
                        _appShutdownTimer.Stop();
                        _appShutdownTimer = null;
                    ...
                    {
                        string timeStr = $"{(int)remaining.TotalDays}:{remaining.Hours}:{remaining.Minutes}:{remaining.Seconds}";
                        lblStatus.Text = _isVietnameseUi 
                            ? $"Tự động tắt máy sau: {timeStr}" 
                            : $"Auto shutdown in: {timeStr}";
                    }
                };
                _appShutdownTimer.Start();
