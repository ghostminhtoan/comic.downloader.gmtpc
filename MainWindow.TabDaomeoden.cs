...
namespace get_link_manga
{
    public partial class MainWindow : Window
    {
        ...
        private async Task DownloadDaomeodenBookAsync(GalleryItem item, string rootFolder, CancellationToken token, GalleryItem queueItem, ChapterFilter chapterFilter)
        {
            ...
            {
                List<string> cachedLinksToDownload = cachedChapterLinks
                    .Where(IsDaomeodenChapterLink)
                    .Distinct(StringComparer.OrdinalIgnoreCase)
                    .ToList();
                if (chapterFilter != null)
                {
                    var filtered = cachedLinksToDownload
                        .Where(link => chapterFilter.IsMatch(ParseChapterNumber(link)))
                        .ToList();
                    cachedLinksToDownload = FilterPendingChapterLinksFromProcess(rootFolder, DaomeodenSiteFolder, item, filtered);
                }
                else
                {
                    ...
                    if (pendingFromProcess != null)
                    {
                        ...
                        {
                            InitializeChapterProcess(rootFolder, DaomeodenSiteFolder, item, cachedLinksToDownload, preserveExistingDone: false);
                        }
                        else
                        {
                            cachedLinksToDownload = pendingFromProcess;
                        }
                    }
                    else
                    {
                        InitializeChapterProcess(rootFolder, DaomeodenSiteFolder, item, cachedLinksToDownload);
                    }
                }
                ...
                if (queueItem != null)
                {
                    Dispatcher.Invoke(() =>
                    {
                        queueItem.TotalChapters = cachedLinksToDownload.Count;
                        queueItem.CompletedChapters = 0;
                    });
                }

                if (cachedLinksToDownload.Count == 0)
                {
                    if (queueItem != null)
                    {
                        Dispatcher.Invoke(() =>
                ...
                foreach (string chapterLink in cachedLinksToDownload)
                {
                    token.ThrowIfCancellationRequested();
                    ...
                    bool completed = await DownloadDaomeodenChapterAsync(chapterItem, rootFolder, token, queueItem, true);
                    if (completed)
                    {
                        MarkChapterProcessDone(rootFolder, DaomeodenSiteFolder, item, chapterLink);
                        completedFromCache++;
                    }
            ...
            if (chapterFilter != null)
            {
                var filtered = chapterLinks
                    .Where(link => chapterFilter.IsMatch(ParseChapterNumber(link)))
                    .ToList();
                chapterLinks = FilterPendingChapterLinksFromProcess(rootFolder, DaomeodenSiteFolder, item, filtered);
            }
            else
            {
                var pendingFromProcess = LoadPendingChapterLinksFromProcess(rootFolder, DaomeodenSiteFolder, item);
